...
class ImportCallc extends Command
{
  ...
  public function handle()
  {
    ...
    foreach ($imports as $import) {
      $a_hm = (object) [];
      if ($import->message != '') {
        $a_hm = DB::selectOne('SELECT SUM(status_id = 8)hmnew,SUM(status_id = 9)hmcb,SUM(status_id = 10)hmdp, SUM(pending) hmpnd, SUM(status_id = 32) hmpot, COUNT(*)hm FROM lids l WHERE l.`load_mess` = "' . $import->message . '"');
      }
    ...
    foreach ($imports_provider as $import) {
      ...
      $a_hm = DB::selectOne('SELECT SUM(status_id = 8)hmnew,SUM(status_id = 9)hmcb,SUM(status_id = 10)hmdp,SUM(pending) hmpnd, SUM(status_id = 32) hmpot, COUNT(*)hm FROM lids l WHERE l.`id` IN (' . implode(',', $lid_ids) . ')');
      if ($a_hm) {
        $a_hm->callc = 0;
        $a_hm->updated_at = date('Y-m-d H:m:s');
